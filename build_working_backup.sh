...
export MAKE_ARGS=$*
export MAKE_FLAGS="-j$(nproc) ${MAKE_FLAGS}"
...
if [ -n "${CC}" ]; then
  CC_ARG="CC=${CC} HOSTCC=${HOSTCC}"
  #CC_ARG="CC=${CC} HOSTCC=gcc"
fi
...
echo "TJR -- LD ${LD}"
echo "TJR -- CC ${CC}"
echo "TJR -- CC_LD_ARG ${CC_LD_ARG}"
echo "TJR -- MAKEFLAGS ${MAKE_FLAGS}"
...
if [ -z "${SKIP_MRPROPER}" ] ; then
  ...
  (cd ${KERNEL_DIR} && \
    make -j32 \
    O=${OUT_DIR} \
    XDTC_EXT=${ROOT_DIR}/prebuilts-master/misc/linux-x86/dtc/dtc \
    XDTC_OVERLAY_TEST_EXT=${ROOT_DIR}/prebuilts-master/misc/linux-x86/libufdt/ufdt_apply_overlay \
    XCONFIG_BUILD_ARM64_DT_OVERLAY=y \
    ...
    HOSTCFLAGS="-I${ROOT_DIR}/kernel/msm-4.14/include/uapi -I/usr/include -I/usr/include/x86_64-linux-gnu -L/usr/lib -L/usr/lib/x86_64-linux-gnu" \
    HOSTLDFLAGS="-L/usr/lib -L/usr/lib/x86_64-linux-gnu" \
    XKERNEL_DEFCONFIG=surfaceduo_defconfig \
    XARCH=arm64 \
    CROSS_COMPILE=${ROOT_DIR}/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9/bin/aarch64-linux-androidkernel- \
    REAL_CC=${ROOT_DIR}/proprietary/llvm-arm-toolchain-ship/8.0/bin/clang CLANG_TRIPLE=aarch64-linux-gnu-make \
    mrproper)
  set +x
fi
...
if [[ -z "${SKIP_EXT_MODULES}" ]] && [[ "${EXT_MODULES}" != "" ]]; then
  ...
  echo HEY got some modules - ${EXT_MODULES}
